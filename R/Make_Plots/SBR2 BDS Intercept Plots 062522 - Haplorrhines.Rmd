---
title: "SB R2 BDS ms Intercept Plots - Haplorhines"
output: html_notebook
editor_options: 
  chunk_output_type: console
---
Haplorhines Analysis
...
load("/Users/markgrabowski/Documents/Academic/Research/Current Projects/Brain Size Diet + Social System/SB Revision 2/Results/SIMMAPS 062522 v1/SIMMAP ACE/Haplorhines/SBR2 - WS from SIMMAP ACE - Haplorhines - Scaled 062522.RData")
...
ms.plot<-ms.plot+geom_abline(intercept = intercepts[1,1],slope=slope,color=reg.colors[4],linetype=2,size=0.75)
ms.plot<-ms.plot+geom_abline(intercept = intercepts[2,1],slope=slope,color=reg.colors[5],linetype=2,size=0.75)
...
ms.plot<-ms.plot+geom_abline(intercept = intercepts[7,1],slope=slope,color=reg.colors[5],linetype=1,size=0.75)

single.regimes<-c("Folivorous Polygynandry","Folivorous Harem Polygyny","Non-folivorous Spatial Polygyny","Non-folivorous Monogamy","Non-folivorous Polyandry","Non-folivorous Polygynandry", "Non-folivorous Harem Polygyny")
...
ms.plot<-ggplot(data=BoM.category.means,aes(x=Categories,y=pred.ECV))+
  ...
  geom_point(data=BoM.category.means,mapping=aes(fill=Categories,shape=Categories),size=5,color="white")+ggtitle("Haplorrhines")
...
Fig S5b: Hap Mating System
```{r}
#Combine external coding and internal coding to plot tree with colored shifts
reg.colors<-pal_aaas()(length(unique(as.factor(BrBo.Diet.DC.GS.SS.AP.trdata$dat$Mating.System.R1))))

regimes<-c(as.character(BrBo.Diet.DC.GS.SS.AP.trdata$dat$Mating.System.R1),as.character(internal.regimes.ms))

edge.regimes <- factor(regimes[BrBo.Diet.DC.GS.SS.AP.trdata$phy$edge[,2]])

plot(BrBo.Diet.DC.GS.SS.AP.trdata$phy,edge.color = reg.colors[edge.regimes], edge.width = 1.75, cex = 0.000001)

#6X4
```

Fig. S5a: Hap Mating System
```{r}
#levels(unique(as.factor(BrBo.Diet.DC.GS.SS.AP.trdata$dat$Mating.System.R1)))
#[1] "Harem Polygyny"   "Monogamy"         "Polyandry"        "Polygynandry"     "Spatial Polygyny"

ms.plot<-ggplot(BrBo.Diet.DC.GS.SS.AP.trdata$dat,aes(y=log(ECV.species.mean),x=log(Body.mass.species.mean)))
                                                                                                BrBo.Diet.DC.GS.SS.AP.trdata$dat$Mating.System.R1
ms.plot<-ms.plot+geom_point(aes(fill=Mating.System.R1),pch=21,size=3.5,color="white") 
                                                                        
ms.plot<-ms.plot+xlab("log Body Mass (g)")+ylab("log Endocranial Volume (cc)")
#ms.plot<-ms.plot+theme(axis.text=element_text(size=24),axis.title=element_text(size=24))
                           ms.plot<-ms.plot+theme(axis.text=element_text(size=16),axis.title=element_text(size=16),legend.title=element_text(size=10),legend.text=element_text(size=12),
...
ms.plot<-ms.plot+theme(legend.key = element_rect(fill = "white"),legend.title = element_text(size=14) )
#+scale_x_continuous(expand=c(0.5,0))
ms.plot<-ms.plot+scale_fill_aaas()

#Haplorrhine Trends - Revised Version - 5 inch by 5 inch plots, then increase in size.
int.1<- ECV.MS.BoM.BM$beta_primary$coefficients[1] #Haram polygyny, 
int.2<- ECV.MS.BoM.BM$beta_primary$coefficients[2] #Monogamy
int.3<- ECV.MS.BoM.BM$beta_primary$coefficients[3] #Polyandry
int.4<- ECV.MS.BoM.BM$beta_primary$coefficients[4] #Polygynandry
int.5<- ECV.MS.BoM.BM$beta_primary$coefficients[5] #Spatial Polygyny  
slope<- ECV.MS.BoM.BM$beta_primary$coefficients[6] #Body mass



ms.plot<-ms.plot+geom_abline(intercept = int.1,slope=slope,color=reg.colors[1],size=0.75)
ms.plot<-ms.plot+geom_abline(intercept = int.2,slope=slope,color=reg.colors[2],size=0.75)
ms.plot<-ms.plot+geom_abline(intercept = int.3,slope=slope,color=reg.colors[3],size=0.75)
ms.plot<-ms.plot+geom_abline(intercept = int.4,slope=slope,color=reg.colors[4],size=0.75)
ms.plot<-ms.plot+geom_abline(intercept = int.5,slope=slope,color=reg.colors[5],size=0.75)


ms.plot<-ms.plot+labs(fill="Mating System")

ms.plot<-ms.plot+guides(fill = guide_legend(override.aes = list(size=5)))


#ms.plot<-ms.plot+labs(col="")
ms.plot
##########
#6X8
```
...
Fig S6b: Hap Diet Categories
```{r}
reg.colors<-pal_aaas()(length(unique(as.factor(BrBo.Diet.DC.GS.SS.AP.trdata$dat$Org.Fol.Non.Fol))))

regimes<-c(as.character(BrBo.Diet.DC.GS.SS.AP.trdata$dat$Org.Fol.Non.Fol),as.character(internal.regimes.FoDC))
...
Fig. S6a: Hap Diet Categories
```{r}
#levels(unique(as.factor(BrBo.Diet.DC.GS.SS.AP.trdata$dat$Org.Fol.Non.Fol)))
#[1] "Fol"     "Non.Fol"

ms.plot<-ggplot(BrBo.Diet.DC.GS.SS.AP.trdata$dat,aes(y=log(ECV.species.mean),x=log(Body.mass.species.mean)))
ms.plot<-ms.plot+geom_point(aes(fill=Org.Fol.Non.Fol),size=3.5,color="white",pch=21)
ms.plot<-ms.plot+xlab("log Body Mass (g)")+ylab("log Endocranial Volume (cc)")
                                       
ms.plot<-ms.plot+scale_fill_aaas()

                                                                                ms.plot<-ms.plot+theme(axis.text=element_text(size=16),axis.title=element_text(size=16),legend.title=element_text(size=14),legend.text=element_text(size=12),
...
ms.plot<-ms.plot+theme(legend.key = element_rect(fill = "white"))



#Haplorrhine Trends - Revised Version - 5 inch by 5 inch plots, then increase in size.
int.1<- -2.422632994 # Folivore
int.2<- -1.251149156 #Non folivore
#log(BrBo.Diet.DC.GS.SS.AP.trdata$dat$Body.mass.species.mean):0.5316


#ms.plot<-ms.plot+geom_abline(intercept = int.1,slope=0.559560102672493,color=reg.colors[1])+geom_dl(aes(label="FHP"),method=list(dl.trans(x=x+0.2),"last.points"))
  
ms.plot<-ms.plot+geom_abline(intercept = int.1,slope=0.592846419,color=reg.colors[1],size=0.75)
ms.plot<-ms.plot+geom_abline(intercept = int.2,slope=0.592846419,color=reg.colors[2],size=0.75)


#ms.plot<-ms.plot+geom_abline(intercept = comp.int,slope=0.5630,color="black")
#ms.plot<-ms.plot+geom_point(aes(x=log(57849.1),y=log(1299.0/1.036)),color="black")
#6X8 Pdf
ms.plot<-ms.plot+labs(fill="Diet Categories")
ms.plot<-ms.plot+guides(fill = guide_legend(override.aes = list(size=5)))
